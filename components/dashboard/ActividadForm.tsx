...
export function ActividadForm({ requiereObservacion }: { requiereObservacion: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div>
        <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Descripción (obligatoria)</label>
        <textarea
          name="descripcion"
          required
          rows={2}
      ...
      {requiereObservacion && (
        <div>
          ...
          <textarea
            ...
            required
            rows={2}
            className="w-full rounded-lg border border-warning/40 bg-warning-bg/40 px-3 py-2 text-sm text-gray-900"
          />
        </div>
      )}
